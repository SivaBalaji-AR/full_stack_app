...
// Define the shape of the meta data from your API
interface MetaData {
    total: number;
    page: number;
    limit: number;
    ...
    useEffect(() => {
        const fetchConsumers = async () => {
            ...
            try {
                ...
                const res = await fetch(`/api/admin/shop?page=${page}&limit=${pageSize}`);
                if (res.ok) {
                            ...
                            shops.map((consumer) => (
                                <TableRow key={consumer.id}>
                                    ...
                                    <TableCell className="truncate max-w-[200px]">
                                        {consumer.admin?.name || 
                                            <span className="text-muted-foreground italic">No admin</span>}
                                    </TableCell>
                                </TableRow>
                            ))
                        )}
